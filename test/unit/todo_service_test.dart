...
void main() {
  ...
  group('TodoService', () {
    ...
    test(
      'getTodos returns a list of todos when response is successful',
      () async {
        // Mock data
        final mockTodosJson = {
          'todos': [
    ...
    test('getTodos throws exception when response times out', () async {
      mockClient.mockGet(
        '${todoService.baseUrl}/todos',
    ...
    test('getTodos throws exception when response is malformed JSON', () async {
      mockClient.mockGet(
        '${todoService.baseUrl}/todos',
    ...
    test(
      'getTodoById throws exception when response is not successful',
      () async {
        // Setup the mock response
        mockClient.mockGet(
          '${todoService.baseUrl}/todos/999',
          http.Response('Not Found', 404),
        );
    ...
    test('getTodoById throws exception when response times out', () async {
      mockClient.mockGet(
        '${todoService.baseUrl}/todos/1',
        http.Response('Timeout', 408),
        delay: Duration(seconds: 5),
      ...
      expect(() => todoService.getTodoById(1), throwsException);
    });

    test(
      'getTodoById throws exception when response is malformed JSON',
      () async {
        mockClient.mockGet(
          '${todoService.baseUrl}/todos/1',
          http.Response('Invalid JSON', 200),
